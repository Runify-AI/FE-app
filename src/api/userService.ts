...
export const sendVerificationCode = async (email: string) => {
  try {
    const response = await axiosInstance.post('/api/send-code', { email });
    return response.data;
  } catch (error: any) {
...
export const findUserEmail = async (nickName: string, runningType: string) => {
  try {
    const response = await axiosInstance.post('/api/find-email', { nickName, runningType });
    return response.data;
  } catch (error: any) {
...
export const confirmResetPassword = async (code: string, newPassword: string) => {
  try {
    const response = await axiosInstance.post('/api/confirm-reset-password', {
      code,
      newPassword,
    });
    return response.data;
...
export const getUserPreferences = async (token: string) => {
  const response = await axiosInstance.get('/api/preferences-list', {
...
// 선호도 조회하기
export const createUserPreferences = async (token: string, preferences: {
  preferencePlaces: string[],
  preferenceRoutes: string[],
  preferenceAvoids: string[],
  preferenceEtcs: string[],
}) => {
  const response = await axiosInstance.post('/api/preferences-create', preferences, {
    headers: { Authorization: `Bearer ${token}` },
  });
  return response.data;
};
...
export const updateUserProfile = async (
  ...
  try {
    const response = await axiosInstance.put('/api/profile-update', profile, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
