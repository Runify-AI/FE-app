// app/user/preference-settings.tsx
...
const PreferenceSettingsScreen = () => {
  const router = useRouter(); // ✅ useNavigation → useRouter
  ...
  const getLabel = (ids: string[], list: PreferenceOption[]) =>
    ids.includes('NONE') ? '상관없음' :
      ids.map(i => list.find(l => l.id === i)?.label).filter(Boolean).join(', ');
  ...
  const handleSave = async () => {
    try {
      ...
      router.back(); //  navigation.goBack() → router.back()
    } catch (e: any) {
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          {renderSection('기타 옵션', additionalOptions, selectedAdditionalOptions, setSelectedAdditionalOptions, 'etcs')}
